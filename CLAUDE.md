...
main.py (FastAPI entry point)
  ...
  POST /webhook/telegram  -> receives auth codes from Telegram
  lifespan:
...
Orchestrator (supervisor_service/ochestrator.py)
  ...
  -> RunController x2 (fyers + penny detector threads)
...
│   │   ├── server.py                # POST /webhook/telegram router
...
│   ├── supervisor_service/
│   │   ├── ochestrator.py           # Manages both detectors + scheduling
...
│   ├── summary_service/
│   │   ├── summary_generator.py     # SummaryGenerator (parameterized per detector)
...
- **Shared token**: One FyersAuthenticator instance, token passed to both detectors
...
- `Dockerfile` uses `python:3.12-slim`, runs `python main.py`
